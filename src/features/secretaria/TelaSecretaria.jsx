  ...
  const provasComAtletas = useMemo(() => {
    ...
    inscsEvt.forEach(insc => {
      ...
      if (!grupos[key]) {
        ...
        // Buscar horário do programa
        const entries = Object.entries(progHorario);
        for (const [hr, lista] of entries) {
          if (Array.isArray(lista) && lista.some(p =>
            typeof p === "object" ? p.provaId === insc.provaId && p.catId === cat.id && p.sexo === (insc.sexo || atl.sexo)
            : false
          )) {
            grupos[key].horario = hr;
            break;
          }
        }
      }
