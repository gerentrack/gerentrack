...
const TeamScoringEngine = {
  ...
  calcularClassificacaoEquipes(eventoAtual, inscricoes, resultados, atletas, equipes, recordes, filtroSexo) {
    ...
    // ── Provas pendentes + recontagem consistente de totais ──
    // Source-of-truth único: combinações (nome+categoria+sexo) com inscrições.
    // Total = nº de combinações; Completas = combinações com pelo menos 1 resultado.
    // Dedup por nome cobre variantes M_/F_ da mesma prova lógica.
    var combinacoesMap = new Map(); // key → { completa: bool, pendente: {..} | null }
    var registrarCombo = function(nome, catId, sexo, completa, itemPendente) {
      var key = nome + "_" + catId + "_" + sexo;
      var prev = combinacoesMap.get(key);
      if (!prev) {
        combinacoesMap.set(key, { completa: !!completa, pendente: completa ? null : itemPendente });
      } else if (!prev.completa && completa) {
        // Achou resultado em alguma variante — promove para completa
        combinacoesMap.set(key, { completa: true, pendente: null });
      }
    };
    ...
    provasPrograma.forEach(function(provaId) {
      ...
      _sexos.forEach(function(sexo) {
        CATEGORIAS.forEach(function(cat) {
          ...
          var completa = !!(resPend && Object.keys(resPend).length > 0);
          var item = completa ? null : {
            provaId: prova.id,
            provaNome: prova.nome,
            categoriaId: cat.id,
          ...
          registrarCombo(prova.nome, cat.id, sexo, completa, item);
        });
      });
    });
    // Combinadas: 1 combinação por prova×cat (sexo derivado do ID)
    provasCombinadas.forEach(function(prova) {
      var sexoProva = prova.id.startsWith("F_") ? "F" : "M";
      if (filtroSexo && sexoProva !== filtroSexo) return;
      ...
      registrarCombo(prova.nome, catIdC, sexoProva, completa, item);
    });
    var provasPendentes = [];
    var combsCompletasCount = 0;
    combinacoesMap.forEach(function(v) {
      if (v.completa) combsCompletasCount++;
      else if (v.pendente) provasPendentes.push(v.pendente);
    });
    // Sobrescreve contadores para alinhar com provasPendentes
    totalProvas = combinacoesMap.size;
